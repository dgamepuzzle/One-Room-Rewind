...
public class ShatterTool : MonoBehaviour
{
	...
	public void Split(Plane[] planes)
	{
		...
		if (planes != null && planes.Length > 0 && hull != null && !hull.IsEmpty)
		{
			if (uvMapper != null)
			{
                ...
                {
                    RewindManager.Instance.RegisterGameObject(gameObject);
                }
                ...
                //Destroy(gameObject);
                RewindManager.Instance.SnapActionToObject(gameObject, () =>
                {
	...
	private void CreateNewHulls(Plane[] localPlanes, out IList<Hull> newHulls)
	{
		...
		// Add the existing hull
		newHulls.Add(new Hull(GetComponent<MeshFilter>().mesh));
